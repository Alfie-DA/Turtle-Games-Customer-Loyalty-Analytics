## LSE Data Analytics Online Career Accelerator 
# DA301:  Advanced Analytics for Organisational Impact

###############################################################################

# Assignment 5 scenario
## Turtle Games’s sales department has historically preferred to use R when performing 
## sales analyses due to existing workflow systems. As you’re able to perform data analysis 
## in R, you will perform exploratory data analysis and present your findings by utilising 
## basic statistics and plots. You'll explore and prepare the data set to analyse sales per 
## product. The sales department is hoping to use the findings of this exploratory analysis 
## to inform changes and improvements in the team. (Note that you will use basic summary 
## statistics in Module 5 and will continue to go into more detail with descriptive 
## statistics in Module 6.)

################################################################################

## Assignment 5 objective
## Load and wrangle the data. Use summary statistics and groupings if required to sense-check
## and gain insights into the data. Make sure to use different visualisations such as scatterplots, 
## histograms, and boxplots to learn more about the data set. Explore the data and comment on the 
## insights gained from your exploratory data analysis. For example, outliers, missing values, 
## and distribution of data. Also make sure to comment on initial patterns and distributions or 
## behaviour that may be of interest to the business.

################################################################################

# Module 5 assignment: Load, clean and wrangle data using R

## It is strongly advised that you use the cleaned version of the data set that you created and 
##  saved in the Python section of the course. Should you choose to redo the data cleaning in R, 
##  make sure to apply the same transformations as you will have to potentially compare the results.
##  (Note: Manual steps included dropping and renaming the columns as per the instructions in module 1.
##  Drop ‘language’ and ‘platform’ and rename ‘remuneration’ and ‘spending_score’) 

## 1. Open your RStudio and start setting up your R environment. 
## 2. Open a new R script and import the turtle_review.csv data file, which you can download from 
##      Assignment: Predicting future outcomes. (Note: You can use the clean version of the data 
##      you saved as csv in module 1, or, can manually drop and rename the columns as per the instructions 
##      in module 1. Drop ‘language’ and ‘platform’ and rename ‘remuneration’ and ‘spending_score’) 
## 3. Import all the required libraries for the analysis and view the data. 
## 4. Load and explore the data.
##    - View the head the data.
##    - Create a summary of the new data frame.
## 5. Perform exploratory data analysis by creating tables and visualisations to better understand 
##      groupings and different perspectives into customer behaviour and specifically how loyalty 
##      points are accumulated. Example questions could include:
##    - Can you comment on distributions, patterns or outliers based on the visual exploration of the data?
##    - Are there any insights based on the basic observations that may require further investigation?
##    - Are there any groupings that may be useful in gaining deeper insights into customer behaviour?
##    - Are there any specific patterns that you want to investigate
## 6. Create
##    - Create scatterplots, histograms, and boxplots to visually explore the loyalty_points data.
##    - Select appropriate visualisations to communicate relevant findings and insights to the business.
## 7. Note your observations and recommendations to the technical and business users.

###############################################################################

# Your code here.
# 2. IMPORT THE CLEAN DATA
df <- read.csv('data/clean_reviews.csv')

# View Data
head(df)

# Structure
str(df)

# Summary
summary(df)

################################################################################
# 3. LOAD REQUIRED LIBRARIES
install.packages(c('ggplot2', 'dplyr'))

library(ggplot2)
library(dplyr)

################################################################################
# 4. BASIC EXPLORATION
# Check column names
colnames(df)

# Check missing values
colSums(is.na(df))

###############################################################################
# 5. EXPLORATORY DATA ANALYSIS (EDA)
# 5a(i) Distribution
ggplot(df, aes(x = loyalty_points)) +
  geom_histogram(bins = 20) +
  ggtitle('Ditribution of Loyalty Points')

# 5a(ii) Patterns/Relationships
# Remuneration vrs Loyalty Points
ggplot(df, aes(x = remuneration, y = loyalty_points)) +
  geom_point() +
  ggtitle('Remuneration vrs Loyalty Points')

# Spending Score vrs Loyalty Points
ggplot(df, aes(x = spending_score, y = loyalty_points)) +
  geom_point() +
  ggtitle('Spending Score vrs Loyalty Points')

# 5a(iii) Outliers
ggplot(df, aes(y = loyalty_points)) +
  geom_boxplot() +
  ggtitle('Boxplot of Loyalty Points')

# 5a (iv) Groupings
# Create groups
df$spending_group <- cut(df$spending_score,
                         breaks = c(0, 30, 70, 100),
                         labels = c('Low', 'Medium', 'High'))
# Compare groups
df %>% 
  group_by(spending_group) %>%
  summarise(avg_loyalty = mean(loyalty_points))

#5b Visualisations as required
# SCATTERPLOT
ggplot(df, aes(x = spending_score, y = loyalty_points)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  ggtitle('Spending Score vrs Loyalty Points')

# BOXPLOT
ggplot(df, aes(x = ' ', y = loyalty_points)) +
  geom_boxplot() +
  ggtitle('Loyalty Points Boxplot')

##########################################################################################################
# OBSERVATIONS & INTERPRETATION
##    - Can you comment on distributions, patterns or outliers based on the visual exploration of the data?
## DISTRIBUTION: HISTOGRAM
## The distribution of loyalty points is right skewed.
## Most customers are clustered around ~500 to 2000.
## There is long tail to the right. Some customers have very high points (3000 - 6500).
## Majority of customers earn moderate loyalty points but are less engaged by the business.
## But a small number of customers accumulate very high loyalty points. They represent high value
## customers but highly engaged.

## DISTRIBUTION: BOXPLOT
## The median is around ~1200 to 1400 points.
## The box (IQR) shows most customers fall between ~700 to 1800 points.
## There are many outliers above 3000.
## There are fewer extreme low outliers.

# DISTRIBUTION: SCATTER PLOTS
# There is a positive relationship between remuneration and spending score vrs loyalty points.
# The relationship is much linear and cleaner between spending score and loyalty points than
# remuneration and loyalty points which appesrs widely scattered especially at higher income levels.
# From the remuneration plot, three (3) distinct customer groups can be identified.
        ## Low income earners (20 - 40) with low loyalty points
        ## Middle income earners (40 - 70) with mixed behaviour of moderate to high loyalty.
        ## High income earners (70 - 90) very wide spread among this group of customers:
               ## some very high loyalty, some very low loyalty. These are outliers.

## INSIGHT
## Both the histogram and boxplot reveal a significant number of outliers, indicating the presence of a small
## but important segment of highly engaged customers. These individuals contribute disproportionately to loyalty
## accumulation and may represent key targets for retention strategies. The relatively tight interquartile range
## suggests that most customers show similar behaviour, while the long upper tail highlights opportunities to
## convert average customers to high_value segments. 
## Key insight: even thought as remuneration increases, loyalty points increases, higher income does not 
## always guarantee high loyalty and that customer spending behaviour is a better predictor of loyalty.

#########################################################################################################
##    - Are there any insights based on the basic observations that may require further investigation?
## What is surprising and require further investigation is from the spending score plot where it can be 
## observed that, there are customer with high spending score but low loyalty. Why are they not highly
## engaged.

########################################################################################################
##    - Are there any groupings that may be useful in gaining deeper insights into customer behaviour?
## Yes: further investigatin into why some higher earners have low loyalty. A possible answer could be
## Turtle Games not offering the right product-mix

#######################################################################################################
##    - Are there any specific patterns that you want to investigate
## Yes: from the histogram, why there is high concentration of customers with low loyalty points < 2000.












###############################################################################
###############################################################################

# Assignment 6 scenario

## In Module 5, you were requested to redo components of the analysis using Turtle Games’s preferred 
## language, R, in order to make it easier for them to implement your analysis internally. As a final 
## task the team asked you to perform a statistical analysis and create a multiple linear regression 
## model using R to predict loyalty points using the available features in a multiple linear model. 
## They did not prescribe which features to use and you can therefore use insights from previous modules 
## as well as your statistical analysis to make recommendations regarding suitability of this model type,
## the specifics of the model you created and alternative solutions. As a final task they also requested 
## your observations and recommendations regarding the current loyalty programme and how this could be 
## improved. 

################################################################################

## Assignment 6 objective
## You need to investigate customer behaviour and the effectiveness of the current loyalty program based 
## on the work completed in modules 1-5 as well as the statistical analysis and modelling efforts of module 6.
##  - Can we predict loyalty points given the existing features using a relatively simple MLR model?
##  - Do you have confidence in the model results (Goodness of fit evaluation)
##  - Where should the business focus their marketing efforts?
##  - How could the loyalty program be improved?
##  - How could the analysis be improved?

################################################################################

## Assignment 6 assignment: Making recommendations to the business.

## 1. Continue with your R script in RStudio from Assignment Activity 5: Cleaning, manipulating, and 
##     visualising the data.
## 2. Load and explore the data, and continue to use the data frame you prepared in Module 5.
## 3. Perform a statistical analysis and comment on the descriptive statistics in the context of the 
##     review of how customers accumulate loyalty points.
##  - Comment on distributions and patterns observed in the data.
##  - Determine and justify the features to be used in a multiple linear regression model and potential
##.    concerns and corrective actions.
## 4. Create a Multiple linear regression model using your selected (numeric) features.
##  - Evaluate the goodness of fit and interpret the model summary statistics.
##  - Create a visual demonstration of the model
##  - Comment on the usefulness of the model, potential improvements and alternate suggestions that could 
##     be considered.
##  - Demonstrate how the model could be used to predict given specific scenarios. (You can create your own 
##     scenarios).
## 5. Perform exploratory data analysis by using statistical analysis methods and comment on the descriptive 
##     statistics in the context of the review of how customers accumulate loyalty points.
## 6. Document your observations, interpretations, and suggestions based on each of the models created in 
##     your notebook. (This will serve as input to your summary and final submission at the end of the course.)

################################################################################

# Your code here.

## 1: LOAD THE DATA
df <- read.csv('data/clean_reviews.csv')

head(df)
str(df)
summary(df)

#############################################################################################
## 2. SELECT FEATURES RELEVANT FOR THE MODEL
## Target Variable: loyalty_points
## Predictors: numeric only - age, remuneration, spending_score, (product: as it is, product variable
## is just an ID CODE = NUMERIC LABEL, not a true numeric variable).

##########################################################################################
## 3. BUILD THE MULTI-LINEAR MODEL

## WITHOUT INCLUSION OF PRODUCT
model <- lm(loyalty_points ~ age + remuneration + spending_score, data = df)

summary(model)

## WITH INCLUSION OF PRODUCT: after conversion to categorical factor.
df$product <- as.factor(df$product)

model_2 <- lm(loyalty_points ~ age + remuneration + spending_score + product, data = df)

summary(model_2) 
## The product variable will now be excluded from the analysis due to its high cardinality,
## resulting in large number of dummy variables when converted to catagorical feature.

#############################################################################################
## 4. INTERPRET MODEL OUTPUT
## The model demonstrates strong explanatory power, with an R-squared value 0f 0.8399.
## This indicates approximately 84% of the variation in loyalty points is explained by 
## age, remuneration and spending score.
## Also the overall model is statistically significant (p < 2.2e-16), confirming the presence
## of meaningful relationships between the predictors and the target variable.
## Among the predictors, spending score and remuneration show the strongest positive effects
## with co-efficients of 34.18 and 34.01 respectively. In contrast, 'age' has relatively smaller impact
## with co-efficient of 11.06. 

## INSIGHT: Customer behaviour especially spending score plays a more important role in driving customer 
## loyalty than demographics like age.

## Visual Demonstration of the Model
par(mfrow = c(2, 2))
plot(model)

## EXPLANATION OF DIAGNOSTIC PLOT OUTPUT
## Residuals vrs Firred: this does not look random. This looks U-shape. Pattern exits.
## Indicates non-linearity.
## Meaning the predictors do not fully explain loyalty points in a staight line.

## Q-Q Residuals: the points are on the line but deviations at the tails. 
## Residuals are not perfectly normally distributed
## Some outliers exist. Meaning model errors are slightly skewed and affects reliability
## of statistical tests.

## Scale-Location: points are not flat horizontal spread. This shows increasing spread.
## Meaning variance is not constant and that prediction errors increase as loyalty points increase.
## Therefore is less stable for high-value customers

## Residual vrs Leverage: detects outliers or influential outliers. 
## This shows some loyalty points with higher leverage exist.
## Meaning few customers are heavily influencing the regression results

## MODEL USEFULNESS
## Model demonstrates moderate explanatory power with spending score emerging as the 
## most significant predictor of loyalty points. However, variability in residuals and the presence of 
## outliers suggest that the model does not fully capture customer behaviour. Therefore, while
## useful for general insights (R-squared = 84%), the diagnostics plots reveal the model has 
## limitations for precise prediction and should be interpreted with caution.
## That is, the model violates key regression assumptions such as non-linearity in this instance.

## DEMONSTRATE HOW MODEL COULD BE USED TO PREDICT
new_customer <- data.frame(
  age = 35,
  remuneration = 60,
  spending_score = 80
)

predict(model, newdata = new_customer)
## The output = (1, 2959): this means a customer (observation #1) with the above characteristics 
## would accumulate ~2959 loyalty points. From earlier analysis, most customers accumulate between
## 500 - 2000 loyalty points and high value customers accumulate over 3000 points. So this 
## predicted customer is in the mid-range. This presents opportunity to the sales team to retain and 
## target with other marketing initiatives.

## LIMITATION
## The model only provides estimates, not exact and explains only 80% (not 100%). Real behaviour may vary.












###############################################################################
###############################################################################




